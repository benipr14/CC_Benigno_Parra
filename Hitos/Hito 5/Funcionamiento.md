...
- Uso principal: ejecutar `scripts/collect_fly_evidence.sh cc-benigno-parra` para recopilar estado, logs y respuestas de endpoints en un directorio `evidence/<APP>-<TIMESTAMP>` listo para adjuntar.
...
2. Salida esperada: el script crea un directorio `evidence/cc-benigno-parra-<TIMESTAMP>/` y escribe varios archivos dentro. El script imprime la ruta del directorio al terminar.

Archivos que genera `collect_fly_evidence.sh`

- `fly_status.txt` — salida de `flyctl status --app cc-benigno-parra`.
