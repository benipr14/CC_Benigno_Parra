...
- Instalación de dependencias:
  - Se usa `requirements-dev.txt` y `pip install --no-cache-dir -r requirements-dev.txt`. Justificación: reusar `requirements-dev.txt` asegura que las mismas dependencias que se usan en desarrollo y pruebas se instalen en la imagen, lo que facilita reproducibilidad y evita divergencias entre entornos.
...
- Mapear puertos diferentes por réplica (8000, 8001, 8002) simplifica pruebas puntuales e inspección directa de cada instancia sin necesidad de un balanceador.
