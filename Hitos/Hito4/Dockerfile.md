...
- `COPY requirements-dev.txt ./` y `RUN pip install --upgrade pip && pip install --no-cache-dir -r requirements-dev.txt`
	- Justificación: instalar dependencias declaradas garantiza que la imagen tenga todas las librerías necesarias. Reusar `requirements-dev.txt` sincroniza el entorno de desarrollo y el de contenedor, facilitando reproducibilidad de tests y ejecución.
